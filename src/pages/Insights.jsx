...
const Insights = () => {
  ...
  const { 
    ...
    insights // <-- Pulling our newly calculated insights from the store
  } = useAppStore();
            ...
            debtMonths > 0 ? (
              ...
              <div className="bg-[#FFF0F0] dark:bg-[#3A1C1C] border border-red-100 dark:border-red-900/30 p-4 rounded-xl flex items-start space-x-3 mt-4 shadow-sm">
                 ...
                 <div>
                   ...
                   <p className="text-xs text-red-800/80 dark:text-red-300/80 mt-1 leading-relaxed">Your recent payments are insufficient to overcome your ₹{totalCCDebt.toLocaleString('en-IN')} debt. Increase payments immediately.</p>
                 </div>
              </div>
            )
          ) : (
          ...
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-500 dark:text-[#a3a3a3] uppercase tracking-widest">Surplus velocity: <span className={`font-bold ${insights?.avgMonthlySurplus > 0 ? 'text-emerald-500' : 'text-red-500'}`}>₹{(insights?.avgMonthlySurplus || 0).toLocaleString('en-IN')}/mo</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {insights?.goalProjections?.length > 0 ? (
              insights.goalProjections.map((goal, idx) => {
                let badgeColor = 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400 border-red-100 dark:border-red-900/30';
                let Icon = AlertTriangle;
                
                if (goal.status === 'On Track') {
                   badgeColor = 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900/30';
                   Icon = CheckCircle2;
                } else if (goal.status === 'Challenging') {
                   badgeColor = 'bg-yellow-50 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-500 border-yellow-100 dark:border-yellow-900/30';
                   Icon = Clock;
                }

                return (
                  <div key={idx} className="bg-white dark:bg-[#0a0a0a] p-4 rounded-xl border border-gray-100 dark:border-[#262626] shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-bold text-[#0F172A] dark:text-gray-200">{goal.name}</p>
                      <p className="text-[10px] text-gray-500 dark:text-[#a3a3a3] uppercase tracking-widest mt-0.5">₹{goal.remaining.toLocaleString('en-IN')} Remaining</p>
                    </div>
              ...
              <p className="text-xs text-gray-500 dark:text-[#a3a3a3] leading-relaxed p-4 bg-gray-50 dark:bg-[#0a0a0a] rounded-xl border border-dashed border-gray-200 dark:border-[#262626] text-center">No active goals or insufficient data to generate projections.</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-[#F8F9FA] dark:bg-[#121212] p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-[#262626]">
          <div className="flex items-start space-x-4">
            ...
            <div>
              ...
              {activeSubs.length > 4 ? (
                <p className="text-xs text-gray-500 dark:text-[#a3a3a3] leading-relaxed mt-2">
                ...
                <p className="text-xs text-gray-500 dark:text-[#a3a3a3] leading-relaxed mt-2">Subscription payload is highly optimized. No wasted or duplicate recurring charges detected in your ledger.</p>
              )}
            </div>
          </div>
        </div>
